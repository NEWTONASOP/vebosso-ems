...
const managerAccent = RoleAccent.manager;
...
export default function ManagerHistoryScreen() {
  ...
  // ── Team members (scoped to this manager's reports) ──────────────────────
  useEffect(() => {
    if (!profile?.id) return;
    void fetchTeamMembers(profile.id);
  }, [profile?.id, fetchTeamMembers]);

  // ── Team day load — managerId REQUIRED so only direct reports appear ─────
  const loadTeamDay = useCallback(async () => {
    if (!profile?.id) return;
    setFetchError(null);
    const [dayRes] = await Promise.all([
      fetchTeamDay(selectedKey, profile.id),
  ...
  }, [profile?.id, selectedKey, fetchTeamDay, fetchTeamMembers]);
            ...
            <View style={styles.emptyWrap}>
              <View style={styles.emptyCard}>
                ...
                <Text style={styles.emptyBody}>
                  See any of your reports&apos; attendance day by day
                </Text>
                <AnimatedPressable
                  scaleTo={0.97}
